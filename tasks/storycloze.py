...
def get_storycloze_dataset(model_args, data_args, training_args, prompt=""):
    print(prompt)
    ...
    for doc in raw_datasets['validation'] :
        ...
        res.append((ctx," {}".format(doc['RandomFifthSentenceQuiz1'])))
        res.append((ctx," {}".format(doc['RandomFifthSentenceQuiz2'])))
        labels.append(doc['AnswerRightEnding']-1)
...
def evaluate_storycloze(model, model_args, data_args, training_args, additional_args):
    eval_dataset, labels, tokenizer = get_storycloze_dataset(model_args, data_args, training_args, prompt=PROMPT_WITH_TYPE[additional_args.eval_prompt_type])
    ...
    for i in range(0, len(results), 2):
        import numpy as np
        preds.append(np.argmax([results[i], results[i + 1]]))
    correct = 0
