...
class L0Module(Module):
    def __init__(self,
        ...
        self.all_types = ["hidden_z", "intermediate_z", "mlp_z", "head_layer_z", "head_z", "layer_z"]
        ...
        self.shapes['layer'] = [self.num_hidden_layers]
    ...
    def initialized_layer_gate(self):
        ...
        self.add_one_module(self.layer_loga, type="layer", 
                            ...
                            shape=[n_layer])
        logger.info(f"Initialized layerwise gate! Prunable_model_size = {self.prunable_model_size}")
    ...
    def calculate_model_size(self, zs):
        ...
        remaining_head_nums = head_z.reshape(self.num_hidden_layers, self.num_attention_heads).sum(-1)
        remaining_head_nums = (self.num_attention_heads - (self.num_attention_heads - remaining_head_nums) * head_layer_z).tolist()
        ...
        if training:
            for i, type in enumerate(self.types):
                ...
                if type != "layer":
                    ...
                    zs[f"mlp_z"] = z.reshape(self.shapes[type])
                    zs[f"head_layer_z"] = z.reshape(self.shapes[type])
