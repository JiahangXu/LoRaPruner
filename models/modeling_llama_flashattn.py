...
from .modeling_llama import LlamaConfig
...
def _ntk_build_inv_freq(dim, base, scaling_factor, ntk_factor, extrapolation_factor, original_max_position_embeddings, device):
    ...
    # for dy-part-ntk + search
    # inv_freq_ntk = 1.0 / (_ntk_find_newbase_ntk(dim, base, ntk_factor) ** (torch.arange(0, dim, 2).float().to(device) / dim))
